...
std::shared_ptr<NFP> NfpWorker::getOuterNfp(NFP& A, NFP& B, int type, bool inside) {
    ...
    if (inside || !A.children.empty()) {
        ...
        // Use MinkowskiSum(Bc, Ac) — pattern=-B, path=A — matching
        // MinkowskiConvolution's argument order. For non-convex B,
        // sweeping -B along convex A edges avoids quad-strip artifacts.
        auto solution = Clipper2Lib::MinkowskiSum(Bc, Ac, true);
...
SheetPlacement NfpWorker::placeParts(std::vector<std::shared_ptr<NFP>> sheets, std::vector<std::shared_ptr<NFP>> parts, const NestConfig& config, int nestindex) {
        ...
        for (i = 0; i < static_cast<int>(parts.size()); i++) {
            ...
            std::vector<RotationCandidate> rotationCandidates;
            {
                ...
                for (j = 0; j < config.rotations; j++) {
                    ...
                    // Rotate to next step
                    auto r = std::make_shared<NFP>(rotatePolygon(*trialPart, 360.0f / config.rotations));
                    r->Rotation = trialPart->Rotation + (360.0f / config.rotations);
    ...
    // Parts that couldn't be placed — heavy penalty
    for (i = 0; i < static_cast<int>(parts.size()); i++) {
        fitness += 100000000.0 * (std::fabs(GeometryUtil::polygonArea(*parts[i])) / totalsheetarea);
    }
